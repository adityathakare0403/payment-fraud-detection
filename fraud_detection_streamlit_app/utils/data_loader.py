...
def fetch_data_from_hbase(limit=None, database="Database 1"):
    ...
    try:
        ...
        table_name = "fraud_test_transactions" if database == "Database 1" else "fraudulent_transactions"
        table = connection.table(table_name)

        rows = table.scan(limit=limit)
        data = []
        for key, row in rows:
            record = {"trans_num" if database == "Database 1" else "Transaction ID": key.decode()}
...
def add_record_to_hbase(data, database="Database 1"):
    ...
    try:
        ...
        row_key = data.pop("trans_num" if database == "Database 1" else "Transaction ID")
